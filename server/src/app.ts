...
class Server {
    ...
    routes(): void {
        ...
        this.app.post('/decodificarMail', async (req, res) => {
            ...
            try {
                ...
                const result1 = await this.queryUsuario(decodificado) as any;
                if (result1.length == 0)
                    res.json(0);
                else
                    res.json(result1[0]);
    ...
    queryUsuario = (decodificado: any) => {
        return new Promise((resolve, reject) => {
            let consulta = 'SELECT usuarios.email FROM usuarios WHERE email="' + decodificado + '"';
